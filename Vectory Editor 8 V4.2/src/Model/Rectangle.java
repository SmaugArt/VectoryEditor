 ...
 * @author user
 */
public class Rectangle extends Line2 {
    public Rectangle(){
        this.type="Rectangle";
    ...
    @Override public void lastclick(double X2,double Y2){
        ...
        MyArray.add(new Dots2(X2,MyArray.get(0).getDotsY()));
    ...
    @Override public void rebornBorderDots(){ //пересчет точек !!!!!!!!!!
       ...
       MaxY=PivotY;
       MinY=PivotY;
       double X;//=(this.X1-PivotX)*Math.cos(Phi)-(this.Y1-PivotY)*Math.sin(Phi)+PivotX; //Копия точки X1
       double Y;//=(this.X1-PivotX)*Math.sin(Phi)+(this.Y1-PivotY)*Math.cos(Phi)+PivotY;
       for (int i=0;i<MyArray.size();i++){
           X=(MyArray.get(i).getDotsX()-PivotX)*Math.cos(Phi)-(MyArray.get(i).getDotsY()-PivotY)*Math.sin(Phi)+PivotX;
           Y=(MyArray.get(i).getDotsX()-PivotX)*Math.sin(Phi)+(MyArray.get(i).getDotsY()-PivotY)*Math.cos(Phi)+PivotY;
           MaxX=MMMM(X,MaxX,2);
           MinX=MMMM(X,MinX,1);
           MaxY=MMMM(Y,MaxY,2);
           MinY=MMMM(Y,MinY,1);
